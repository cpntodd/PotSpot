...
# Resolve LAN IP (fallback when no domain)
LAN_IP=$(ip -4 addr show eth0 2>/dev/null | awk '/inet / {print $2}' | cut -d/ -f1 | head -n1)
if [[ -z "$LAN_IP" ]]; then
    LAN_IP=$(ip -4 addr show | awk '/inet / && !/127\.0\.0\.1/ {print $2}' | cut -d/ -f1 | head -n1)
fi
LAN_IP="${LAN_IP:-127.0.0.1}"

# Resolve domain (optional -- falls back to LAN IP)
if [[ -z "${DOMAIN:-}" ]]; then
    echo ""
    echo -e "${CYAN}Enter your domain name (leave blank to use LAN IP: ${LAN_IP}):${NC}"
...
if [[ -z "$DOMAIN" ]]; then
    ...
    echo -e "${YELLOW}  No domain provided. Serving on ${PUBLIC_URL}${NC}"
    echo -e "${YELLOW}  TLS and OAuth will be unavailable without a domain.${NC}"
else
    USING_DOMAIN=true
...
# Google OAuth (requires a domain)
if $USING_DOMAIN; then
...
MINIO_ENDPOINT=http://${LAN_IP}:9000
...
${DOMAIN} {
    # API reverse proxy (must come before SPA fallback)
    handle /api/* {
...
CADDYEOF
    ...
    # No domain: serve on all interfaces via plain HTTP
    cat > /etc/caddy/Caddyfile << CADDYEOF
# PotSpot -- auto-generated by native-install.sh
# No domain configured -- serving on LAN IP via HTTP (no TLS)
:${CADDY_HTTP_PORT} {
    # API reverse proxy (must come before SPA fallback)
    handle /api/* {
        reverse_proxy localhost:3000
...
# Caddy global port options (domain mode only -- IP mode uses :PORT directly)
if $USING_DOMAIN; then
...
if $USING_DOMAIN; then
    ...
    echo -e "  ${CYAN}No domain configured -- serving via HTTP on LAN IP${NC}"
    echo -e "  ${CYAN}Set up a domain later by editing:${NC}"
    echo -e "  ${CYAN}  /etc/caddy/Caddyfile${NC}"
    echo -e "  ${CYAN}  ${INSTALL_PATH}/.env${NC}"
fi
echo ""
...
if $USING_DOMAIN; then
    ...
    echo -e "  ${YELLOW}OAuth login is unavailable without a domain.${NC}"
fi
